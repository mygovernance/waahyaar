...
class Bunyad_Theme_Block 
{
	...
	public function process()
	{
		 ...
		 */
		$page_blocks = (array) Bunyad::registry()->page_blocks;
		...
		$page_blocks[$this->block_id] = array_merge(
				$this->_options, 
				array(
					'_block_tag' => $this->_block, 
					'_block_id'  => $this->block_id, 
					'_col_relative_width' => $relative_width
				)
		);

		Bunyad::registry()->page_blocks = $page_blocks;
	...
	{	
		...
		<?php if ($heading_type == 'blog'): ?>
			...
			<h4 class="block-heading"><span class="title"><?php echo $this->title_link; ?></span></h4>	
		
		<?php else: ?>
			
			<div <?php Bunyad::markup()->attribs('block-heading-wrap', array('class' => 'block-head-b')); ?>>
				<h4 class="title"><?php echo $this->title_link; ?></h4>
