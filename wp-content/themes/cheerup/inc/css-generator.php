...
class Bunyad_Custom_Css
{
	...
	{	
		...
		if ($google_fonts) {
			...
			if (Bunyad::options()->font_charset) {
				$charsets = implode(',', array_filter(Bunyad::options()->font_charset));
			}
			
			// add to beginning 
			array_unshift(
				$this->css, 
				"@import url('https://fonts.googleapis.com/css?family=" . implode('|', $google_fonts)  
	...
	{		
		if (is_customize_preview() OR apply_filters('bunyad_custom_css_nocache', false) === true) {
