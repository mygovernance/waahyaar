...
class Bunyad_Admin_OptionRenderer
{
	...
	public function _render_checkbox($element) {
		...
		$element['options'] = array_merge(
			array('checked' => esc_html_x('Yes', 'Admin', 'cheerup'), 'unchecked' => esc_html_x('No', 'Admin', 'cheerup')), 
		...
		{ 
			...
			if (isset($element['bg_type'])) 
			{
				...
				$type .= $this->render_select(array(
					...
					'options' => array(
						'repeat' => esc_html_x('Repeat Horizontal and Vertical - Pattern', 'Admin', 'cheerup'),
						'cover'  => esc_html_x('Fully Cover Background - Photo', 'Admin', 'cheerup'),
						'repeat-x' => esc_html_x('Repeat Horizontal', 'Admin', 'cheerup'),
						'repeat-y' => esc_html_x('Repeat Vertical', 'Admin', 'cheerup'),
						'no-repeat' => esc_html_x('No Repeat', 'Admin', 'cheerup'),
					),
				));
			}
			
			$output .= '<a href="" class="remove-image button after-upload'. $classes .'">'. esc_html_x('Remove', 'Admin', 'cheerup') .'</a>';			
	...
	public function render_multiple($element)
	{
		...
		$output = '<div class="element-multiple">' . $fields . '<a href="#">' . esc_html_x('Add More', 'Admin', 'cheerup')  . '</a></div>';
	...
	public function _render_multiple_fields($element, $key = null)
	{
		...
		return '<div class="fields'. ($key == null ? ' default' : '') .'">' . $fields . '<a href="#" class="remove">' . esc_html_x('Remove', 'Admin', 'cheerup') . '</a></div>';
